...
def on_startup():
    ...
    init_db_clean(SportKind, SportSegment, Event, EventSegment, Factor)
    parser = EventsParser()
    initial_updates = parser.get_updates()
    os.environ['current_version'] = str(parser.current_version)
    init_sports(initial_updates.get('sports'), SportKind, SportSegment)
    init_events(initial_updates.get('events'), Event, EventSegment, SportSegment)
    init_event_miscs(initial_updates.get('eventMiscs'), Event, EventSegment)
    init_factors(initial_updates.get('customFactors'), Event, EventSegment, Factor)
    init_event_blocks(initial_updates.get('eventBlocks'), Event, EventSegment, Factor)
